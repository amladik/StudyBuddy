...
commandList = ['-w','-t', '-p','-s','-m','-h','-g','-eg','-es','-el','-ew','-td','-l','-r',]
...
async def on_message(message):
    ...
    if(message.content.startswith('-td')and writer not in todoQuery and writer not in playQueryList and writer not in goalQuery and writer not in questionQueryList and (writer in convoDone)and writer not in addsongQuery and writer not in addsiteQuery and writer not in addStudyQuery):
...
client.run('ODMzMzYyNTI2MjkyMDE3MTgy.YHxPVQ.hin0wBXtA9n5nYQXk1wfzQ6CYHU')